...
export function SiteHeader() {
  return (
    <header className="sticky top-0 z-40 border-b border-(--line) bg-[color:rgba(244,236,216,0.86)]/95 backdrop-blur-2xl">
      <div className="mx-auto grid max-w-[1520px] grid-cols-[auto_1fr_auto] items-center gap-4 px-4 py-3 md:px-6 xl:px-8">
        ...
        <nav className="justify-self-center rounded-full border border-(--line) bg-(--surface-float) px-2 py-1.5 text-sm text-(--ink-muted)">
          <div className="flex items-center gap-1">
          {NAV_LINKS.map((link) => (
            <Link
              key={link.href}
              href={link.href}
                className="rounded-full px-3 py-2 transition-colors hover:bg-(--surface-strong) hover:text-(--ink)"
            >
              {link.label}
            </Link>
          ))}
